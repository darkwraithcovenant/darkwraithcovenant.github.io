...
I really would like to someday build a bassline generator for my music. My plan is to run my basslines, and some well known popular bassline styles, through a machine learning algorithm, and train the raw rhythm and note data using binary notation (where 1 is a note event and 0 is a rest event) using midi note numbers to determine pitch. It will then use this trained data to generate new basslines via a sequencer. It would have parameters for sequence length, note density, style, etc. Clearly, I am no stranger to music made using artifical intelligence, and I am not against the use of AI to create music. While this tool would be technically a type of "AI," it differs in a number of ways from the current type of generative AI that has recently set the tech world ablaze.
<!-- excerpt-end -->
